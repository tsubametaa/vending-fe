...
import { LineChart, BarChart3, Loader2, CheckCircle, AlertTriangle, XCircle } from 'lucide-preact';
...
export default function QueueChart({ data, statistics }: QueueChartProps) {
  ...
  const [activeChart, setActiveChart] = useState<'queueTime' | 'timeline' | 'distribution'>('queueTime');
            ...
            datasets: [
              {
                ...
                backgroundColor: data.map(d => 
                  d.timeInQueue > 2 ? 'rgba(239, 68, 68, 0.7)' :
                  d.timeInQueue > 1 ? 'rgba(234, 179, 8, 0.7)' : 'rgba(34, 197, 94, 0.7)'
                ),
                borderColor: data.map(d => 
                  d.timeInQueue > 2 ? 'rgb(239, 68, 68)' :
                  d.timeInQueue > 1 ? 'rgb(234, 179, 8)' : 'rgb(34, 197, 94)'
                ),
                borderWidth: 1,
                borderRadius: 4,
              },
  ...
  return (
    <div>
      ...
      <div class="h-80 md:h-96 bg-dark-300/30 rounded-xl p-4">
        {!chartLoaded ? (
      ...
      <div class="mt-4 grid md:grid-cols-3 gap-4">
        <div class="p-3 rounded-lg bg-success-500/10 border border-success-500/20 text-center">
        ...
        <div class="p-3 rounded-lg bg-warning-500/10 border border-warning-500/20 text-center">
          <div class="flex items-center justify-center gap-2 text-sm text-white/60 mb-1">
            <AlertTriangle class="w-4 h-4 text-warning-400" />
            Queue Time 1-2 min
          </div>
          <div class="text-lg font-bold text-warning-400">
            {data.filter(d => d.timeInQueue >= 1 && d.timeInQueue < 2).length} pelanggan
          </div>
        </div>
        <div class="p-3 rounded-lg bg-danger-500/10 border border-danger-500/20 text-center">
          <div class="flex items-center justify-center gap-2 text-sm text-white/60 mb-1">
