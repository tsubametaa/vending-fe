...
export default function SimulationForm() {
  ...
  const validateForm = (): boolean => {
    ...
    if (formData.lambda >= formData.mu) {
      newErrors.general = 'Lambda (λ) harus lebih kecil dari Mu (μ) untuk sistem stabil';
    }
  ...
  const handleChange = (field: keyof FormData) => (e: Event) => {
    ...
    const value = field === 'playerName' ? target.value : parseFloat(target.value) || 0;
  ...
  const handleSubmit = async (e: Event) => {
    ...
    try {
      const response = await fetch('https://vending-be.vercel.app/api/simulate', {
        method: 'POST',
        headers: {
      ...
      simulationStore.setError(error instanceof Error ? error.message : 'Unknown error');
      setErrors({ general: error instanceof Error ? error.message : 'Simulation failed' });
    } finally {
      setIsLoading(false);
  ...
  const presets = [
    { name: 'Rush Hour', icon: Train, lambda: 0.8, mu: 1.0, customers: 30, desc: 'Jam sibuk stasiun' },
    { name: 'Relaxed Park', icon: TreeDeciduous, lambda: 0.3, mu: 0.6, customers: 15, desc: 'Taman santai' },
    { name: 'Overloaded', icon: AlertTriangle, lambda: 0.9, mu: 1.0, customers: 25, desc: 'Sistem hampir penuh' },
    { name: 'Efficient', icon: Zap, lambda: 0.4, mu: 1.2, customers: 20, desc: 'Server cepat' },
  ];
  ...
  return (
    <div class="card h-full">
      ...
      <div class="mb-6">
        <label class="block text-sm font-bold text-white/70 mb-3 pixel-text">Quick Presets</label>
        <div class="grid grid-cols-2 gap-3">
          {presets.map((preset, i) => (
            ...
            >
              <div class={`flex items-center gap-2 text-sm font-bold transition-colors pixel-text ${
                selectedPreset === preset.name ? 'text-primary-300' : 'group-hover:text-primary-300'
              }`}>
      ...
      <form onSubmit={handleSubmit} class="space-y-5">
        ...
        <div>
          <label class="block text-sm font-medium text-white/80 mb-2">
            <span class="text-primary-400">λ</span> Arrival Rate (pelanggan/menit)
          </label>
          <input
            type="number"
            step="0.1"
        ...
        <div>
          <label class="block text-sm font-medium text-white/80 mb-2">
            <span class="text-accent-400">μ</span> Service Rate (pelanggan/menit)
          </label>
          <input
            type="number"
            step="0.1"
          ...
          />
          {errors.mu && (
            <p class="text-danger-400 text-xs mt-1">{errors.mu}</p>
          )}
          <p class="text-xs text-white/40 mt-1">
        ...
        <div class="p-4 rounded-xl bg-dark-300/50 border border-white/5">
          ...
          <div class="w-full h-2 bg-dark-400 rounded-full overflow-hidden">
            <div 
              class={`h-full transition-all duration-500 ${
                utilization >= 1 ? 'bg-danger-500' :
                utilization >= 0.8 ? 'bg-warning-500' :
                utilization >= 0.5 ? 'bg-primary-500' : 'bg-success-500'
              }`}
        ...
        {errors.general && (
          <div class="p-3 rounded-lg bg-danger-500/20 border border-danger-500/30 text-danger-400 text-sm flex items-center gap-2">
