import type { SimulationData, SimulationStatistics } from './stores/simulationStore';
import { Table, Lightbulb, Info, Download, FileSpreadsheet, X, Loader2, CheckCircle, AlertTriangle, ExternalLink, Copy } from 'lucide-preact';
...
function ExportModal({ isOpen, onClose, data, statistics }: ExportModalProps) {
  ...
  const handleExport = async () => {
    ...
    try {
      const response = await fetch('https://vending-be.vercel.app/api/export-to-sheets', {
        method: 'POST',
      ...
      if (result.success) {
        ...
        setErrorMessage(result.message || 'Export gagal. Pastikan spreadsheet sudah di-share dengan service account.');
      }
    } catch (error) {
      setExportStatus('error');
  ...
  const handleDownloadCSV = () => {
    const headers = [
      'Customer #', 'U1 (Random)', 'X1 (Interarrival)', 'U2 (Random)', 'X2 (Service)',
      'Arrival Time', 'Arrival (Jam)', 'Begin Service', 'Begin (Jam)', 'Service Time',
      'Departure Time', 'Departure (Jam)', 'Time in Queue', 'Time in System'
    ];
    ...
    rows.push([
      'Average', '-', statistics.avgInterarrivalTime.toFixed(4), '-', '-',
      '-', '-', '-', '-', statistics.avgServiceTime.toFixed(4),
      '-', '-', statistics.avgTimeInQueue.toFixed(4), statistics.avgTimeInSystem.toFixed(4)
    ]);
  ...
  const copyToClipboard = () => {
    const headers = [
      'Customer #', 'U1', 'X1', 'U2', 'X2', 'Arrival', 'Begin', 'Service', 'Departure', 'Queue Time', 'System Time'
    ];
  ...
  return (
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div class="relative w-full max-w-lg bg-dark-200 rounded-2xl border border-white/10 shadow-2xl overflow-hidden">
        ...
        <div class="p-6 space-y-6">
          <div class="grid grid-cols-2 gap-3">
            ...
            >
              ...
              <div class="text-left">
                <div class="font-semibold text-success-300">Download CSV</div>
          ...
          <div class="relative">
            ...
            <div class="relative flex justify-center">
              <span class="px-3 bg-dark-200 text-sm text-white/40">atau</span>
            </div>
          </div>
            ...
            <div class="p-4 rounded-xl bg-dark-300/50 border border-white/5 space-y-4">
              ...
              {sheetId && (
                <div class="flex items-center gap-2 text-sm text-success-400">
                  ...
                  <span>Sheet ID terdeteksi: <code class="bg-dark-400 px-2 py-0.5 rounded">{sheetId.slice(0,20)}...</code></span>
                </div>
              )}
                ...
                <div class="flex items-start gap-2 text-sm">
                  ...
                  <div class="text-warning-300">
                    ...
                    <p class="text-white/60">
                      Pastikan spreadsheet sudah di-share dengan service account email dari Google Cloud. 
                      Data akan ditulis ke sheet "Simulation Results".
                    </p>
                  </div>
                </div>
              </div>
              ...
              {exportStatus === 'success' && (
                <div class="p-3 rounded-lg bg-success-500/10 border border-success-500/20">
              ...
              {exportStatus === 'error' && (
                <div class="p-3 rounded-lg bg-danger-500/10 border border-danger-500/20">
    ...
    <div>
      ...
      <div class="mb-4 p-3 rounded-lg bg-primary-500/10 border border-primary-500/20 text-sm flex items-start gap-2">
        ...
        <p class="text-primary-300">
          Format tabel ini identik dengan spreadsheet simulasi akademik.
            ...
            {data.map((row, index) => (
              ...
              >
                ...
                <td class={`px-3 py-2 text-right font-mono whitespace-nowrap ${
                  row.timeInQueue > 2 ? 'text-danger-400' : 
                  row.timeInQueue > 1 ? 'text-warning-400' : 'text-success-400'
                }`}>
      ...
      <div class="mt-4 p-4 rounded-lg bg-dark-300/30 text-xs">
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 gap-2 text-white/60">
          <div><span class="text-primary-300">U1</span>: Random number stream 1 (kedatangan)</div>
          <div><span class="text-primary-300">X1</span>: Interarrival time = -ln(U1)/λ</div>
          <div><span class="text-accent-300">U2</span>: Random number stream 2 (pelayanan)</div>
          <div><span class="text-accent-300">X2</span>: Service time = -ln(U2)/μ</div>
          <div><span class="text-warning-300">Time in Queue</span>: Waktu tunggu dalam antrian</div>
          <div><span class="text-success-300">Time in System</span>: Total waktu dalam sistem</div>
        </div>
      </div>
