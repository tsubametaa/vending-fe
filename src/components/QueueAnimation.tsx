...
export default function QueueAnimation() {
  ...
  const initializeQueue = () => {
    ...
    const people: QueuePerson[] = state.results.simulationData.map(
      (data, index) => ({
    ...
    const animate = (currentFrameTime: number) => {
      ...
      setQueuePeople((prev) =>
        prev.map((person) => {
          ...
          const { arrivalTime, beginServiceTime, departureTime } =
            person.customerData;
    ...
    >
        ...
        {state.results && (
          <div class="flex items-center gap-2">
            <motion.button
              ...
              whileTap={{ scale: 0.9 }}
              onClick={togglePause}
              class="p-2.5 rounded-lg bg-gradient-to-br from-primary-500/30 to-primary-500/10 hover:from-primary-500/50 hover:to-primary-500/20 border-2 border-primary-500/40 transition-all shadow-lg"
            ...
            <motion.button
              ...
              class="p-2.5 rounded-lg bg-gradient-to-br from-accent-500/30 to-accent-500/10 hover:from-accent-500/50 hover:to-accent-500/20 border-2 border-accent-500/40 transition-all shadow-lg"
              title="Restart"
            >
      ...
      {state.results && (
        <motion.div
          ...
          class="mb-4 p-4 rounded-lg bg-gradient-to-r from-dark-300/70 via-dark-300/50 to-dark-300/70 border-2 border-primary-500/30 flex items-center justify-between shadow-lg"
        >
          <div class="flex items-center gap-4">
            <div>
              <span class="text-xs text-white/50 pixel-text">
                Waktu Simulasi
              </span>
              <div class="font-mono text-xl font-bold text-primary-400 pixel-text">
          ...
          <div class="text-right">
            ...
            <div class="font-mono text-xl font-bold text-warning-400 pixel-text">
        ...
        <div class="absolute inset-0">
          ...
          {state.results && (
            <motion.div
              ...
              class="absolute top-4 right-4 bg-gradient-to-br from-dark-400/95 to-dark-300/95 backdrop-blur-sm px-4 py-2 rounded-lg border-2 border-primary-500/40 shadow-xl"
            >
              <span class="text-sm font-bold text-white pixel-text">
                {getSceneName()}
              </span>
            </motion.div>
          )}
        ...
        {!state.results ? (
          <div class="absolute inset-0 flex flex-col items-center justify-center text-center p-4">
            ...
            >
              ...
              <p class="text-white text-xl font-bold pixel-text mb-2">
                Atur parameter dan jalankan simulasi
              </p>
          ...
          <div class="absolute inset-0 flex items-end pb-4">
            <AnimatePresence>
              {servingPerson && (
                <motion.div
                  key={servingPerson.id}
                  initial={{ opacity: 0, scale: 0.5, x: 50 }}
                  animate={{ opacity: 1, scale: 1, x: 0 }}
                  exit={{ opacity: 0, scale: 0.5, x: -50 }}
                  class="absolute left-[15%] md:left-[18%] bottom-[15%] z-20"
                >
                  <div class="relative pixel-character">
                    ...
                    <motion.div
                      ...
                      class="absolute -top-6 left-1/2 transform -translate-x-1/2 bg-success-500 text-white text-xs px-3 py-1 rounded-full whitespace-nowrap shadow-lg border-2 border-success-300 pixel-text"
                    >
            ...
            <div class="absolute right-4 left-[35%] md:left-[40%] bottom-[12%] flex items-end justify-start gap-1 md:gap-2 z-10 overflow-hidden">
              <AnimatePresence mode="popLayout">
                {waitingPeople.slice(0, 6).map((person, index) => (
                  <motion.div
                    ...
                    class="relative pixel-character"
                    style={{ zIndex: 6 - index }}
                  >
                    <img
                      src={person.character}
                      alt={`Customer ${person.id}`}
                      class="h-12 md:h-20 w-auto drop-shadow-[0_2px_4px_rgba(0,0,0,0.6)] pixel-art hover:scale-110 transition-transform scale-x-[-1] animate-queue-walk"
              ...
              {waitingPeople.length > 6 && (
                <motion.div
                  ...
                  animate={{ scale: 1, opacity: 1 }}
                  class="flex items-center justify-center h-12 md:h-20 px-3 bg-gradient-to-br from-primary-500/90 to-accent-500/90 rounded-lg border-2 border-white/30 shadow-lg pixel-badge"
                >
      ...
      {state.results && currentTime > 0 && (
        <div class="mt-4 grid grid-cols-3 gap-4">
          <motion.div
            ...
            class="text-center p-4 rounded-lg bg-gradient-to-br from-primary-500/20 to-primary-500/5 border-2 border-primary-500/40 shadow-lg hover:shadow-primary-500/30 transition-all hover:scale-105"
          >
            <div class="flex items-center justify-center gap-2 text-3xl font-bold text-primary-400 pixel-text">
              <CheckCircle class="w-6 h-6" />
              {donePeople.length}
            </div>
            <div class="text-sm text-white/70 mt-1 pixel-text">
              Selesai Dilayani
            </div>
          </motion.div>
          <motion.div
            ...
            class="text-center p-4 rounded-lg bg-gradient-to-br from-warning-500/20 to-warning-500/5 border-2 border-warning-500/40 shadow-lg hover:shadow-warning-500/30 transition-all hover:scale-105"
          >
          ...
          <motion.div
            ...
            class="text-center p-4 rounded-lg bg-gradient-to-br from-accent-500/20 to-accent-500/5 border-2 border-accent-500/40 shadow-lg hover:shadow-accent-500/30 transition-all hover:scale-105"
          >
